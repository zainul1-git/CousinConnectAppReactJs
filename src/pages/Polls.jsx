...
export default function Polls() {
  ...
  const handleVote = async (pollId, optionId) => {
    try {
      ...
      // Agar already voted hai to bhi list refresh kar dein
      fetchPolls();
    }
  };
  ...
  return (
    <div className="max-w-xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-800 mb-1">🗳️ Polls</h1>
      ...
      {showForm && (
        <form onSubmit={handleCreatePoll} className="bg-white rounded-xl border border-gray-200 p-4 mb-6 space-y-3">
          <input
            type="text"
            value={question}
          ...
          <button type="button" onClick={addOptionField} className="text-sm text-indigo-600 font-medium">
            + Add option
          </button>
          <button
            type="submit"
